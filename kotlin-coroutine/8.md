...
* **코루틴이 상속하지 않는 유일한 코루틴 컨텍스트**
  * 자식 코루틴은 부모 코루틴으로부터 코루틴 컨텍스트를 상속받는데(ex.이름), job은 상속되지 않음
...
fun main() = runBlocking {
    ...
    launch(name + job) {
        ...
        println(childName == name) // true, 이름 상속됨
...
```kotlin
fun main() = runBlocking {
    launch(Job()) { // 새로운 job이 부모로부터 상속받은 부모 job을 대체함
        delay(1000)
...
suspend fun main() = coroutineScope {
    ...
    // complete을 명시적으로 해줬기 때문에 프로그램 종료
}
...
* completeExceptionally(exception: Throwable) : Boolean
  ...
  * 주어진 예외는 CancellationException 으로 래핑 > 즉시 취소됨
...
suspend fun main() = coroutineScope {
    ...
    launch {
        delay(500)
        job.completeExceptionally(Error("Some error")) // 즉시 취소
    }
